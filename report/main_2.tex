...
\subsection{Overhead Experimental}

Para aislar el costo extra de paralelizar, se calculo el overhead experimental
como:

\[
    T_o(p)=pT_p-T_s,\qquad R_o(p)=\frac{T_o(p)}{T_s}
\]

Esta metrica mide cuanto trabajo efectivo adicional aparece al usar $p$
procesos: comunicacion, sincronizacion, reduccion de grillas y trafico de
memoria. Si $R_o(p)$ es pequeno, el costo paralelo se amortiza; si crece mucho,
el overhead domina al computo local.

\begin{figure}[t]
    \centering
    \includegraphics[width=0.92\linewidth]{overhead_ratio_vs_processes.png}
    \caption{Overhead relativo $T_o/T_s$ por cantidad de procesos y resolucion.}
    \label{fig:overhead-ratio}
\end{figure}

La Figura~\ref{fig:overhead-ratio} muestra que el overhead relativo crece con
$p$. Para $r=128$, $R_o$ pasa aproximadamente de $0.15$ en $p=2$ a $0.56$ en
$p=4$ y $2.81$ en $p=8$. Esto explica por que el speedup deja de crecer: aunque
el termino de computo $C/p$ disminuye, los costos de reduccion, sincronizacion y
trafico de memoria aumentan. La misma tendencia aparece en resoluciones menores,
donde el computo local es pequeno y el overhead se vuelve visible mas rapido.
